		...
		<view>
			<view class="formBox">
				<u--form labelPosition="left" :model="form" :rules="rules" ref="form">
					...
					<div class="margin-top-xl"></div>
					<view class="flex">
						<u-checkbox-group v-model="checkboxValue1" placement="column"><u-checkbox customStyle="background:#fff" label=" " :name="1"></u-checkbox></u-checkbox-group>
						<view class="text-blue" @click="showAC">《用户服务协议》及《隐私政策》</view>
						<u-modal :show="uModel" title="《用户服务协议》及《隐私政策》" :asyncClose="true" :showConfirmButton="true" @confirm="confirm">
							<scroll-view scroll-y class="slot-content  maxH"><rich-text :nodes="content"></rich-text></scroll-view>
						</u-modal>
					</view>
				</u--form>
				<!-- <u-toast ref="uToast"></u-toast> -->
				<div class="margin-tb-xl loginBtn">
					<u-button shape="circle" color="linear-gradient(to bottom, #4ABFFD , #0B93FC)" type="primary" customStyle="fontSize:18px;fontWeight:500;" @click="submit">
						注&emsp;&emsp;&emsp;册
					</u-button>
				</div>
			</view>
...
<script>
import { registerApi, getACapi } from '@/config/login.js';
	...
	methods: {
		...
		submit() {
			...
			if (this.checkboxValue1.length < 1) {
				uni.$u.toast('请先同意《用户服务协议》及《隐私政策》');
				return;
			}
			let _this = this;
			this.$refs.form
